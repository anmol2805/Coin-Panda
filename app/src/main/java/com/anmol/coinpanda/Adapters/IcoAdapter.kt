...
//        }
        ...
        Glide.with(c).load(coindata.twitter_url+ "/profile_image?size=original").listener(object : RequestListener<Drawable>{
            override fun onLoadFailed(e: GlideException?, model: Any?, target: Target<Drawable>?, isFirstResource: Boolean): Boolean {
                Glide.with(c).load(R.drawable.imgno).into(holder.image)
                return true
            }
